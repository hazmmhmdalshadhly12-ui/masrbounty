...
export async function verifyDomainAction(formData: FormData): Promise<ActionResult> {
  try {
    ...
    // Surface expired separately; still revalidate so UI shows expired badge
    if (res.error && res.data?.status === 'expired') {
      revalidatePath('/company/onboarding');
      revalidatePath('/company/settings');
...
export async function reverifyDomainAction(formData: FormData): Promise<ActionResult> {
  try {
    const domainId = String(formData.get('domain_id') ?? '').trim();
    if (!domainId) return { ok: false, error: 'معرّف النطاق مفقود' };
    // re-verify is same as verify — verifyCompanyDomain already handles pending/failed/expired reset
    const res = await verifyCompanyDomain(domainId);
    if (res.error && res.data) {
      revalidatePath('/company/onboarding');
      revalidatePath('/company/settings');
